...
class OffPolicyWorkerWithCost(object):
    ...
    def sample(self):
        ...
        for _ in range(int(self.batch_size/self.num_agent)):
            ...
            for i in range(self.num_agent):
                batch_data.append((self.obs[i].copy(), action[i].numpy(), reward[i], obs_tp1[i].copy(), self.done[i], velo))
            self.obs = self.env.reset()
    ...
    def random_sample(self, policy='default'):
        ...
        for _ in range(int(self.batch_size/self.num_agent)):
            ...
            processed_rew = self.preprocessor.process_rew(reward, self.done)
            for i in range(self.num_agent):
                batch_data.append((self.obs[i].copy(), action[i], reward[i], obs_tp1[i].copy(), self.done[i], velo))
            self.obs = self.env.reset()
